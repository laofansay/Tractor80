...
export default function Home() {
  ...
  const dealCards = () => {
    ...
    const dealNextCard = () => {
      ...
      const card = obsCards.shift(); // 取出第一张牌
      ...
      if (card) {
        ...
        console.log('发牌:', card);
        declareTrump(position, card); // 亮主逻辑
    ...
    // 处理发牌完成后的逻辑
    const handleAfterDealing = () => {
      // 确保使用最新的 gamePhase 和 dealerPosition 状态
      if (dealerPosition === null) {
        setGamePhase('trumpSelection'); // 进入亮主阶段
      } else {
        setGamePhase('pickBottomCards'); // 进入拾底牌阶段
      }
    };
