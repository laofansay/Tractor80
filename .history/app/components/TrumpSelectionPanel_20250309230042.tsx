...
type TrumpSelectionPanelProps = {
  ...
  redUpLevel: string;
  blueUpLevel: string;
  onUpgrade: (camp: 'red' | 'blue') => void;
};

export function TrumpSelectionPanel({ onSelectTrump, gamePhase, availableSuits, redUpLevel, blueUpLevel, onUpgrade }: TrumpSelectionPanelProps) {
  // 只在亮主阶段显示
      ...
      <div className="flex flex-col gap-2">
        <div className="flex flex-row gap-2">
          <div className="flex flex-col gap-1 mr-2">
            <button
              onClick={() => onUpgrade('red')}
              className="w-10 h-10 bg-white hover:bg-slate-100 text-black rounded-md transition-colors flex items-center justify-center"
              title="红队升级"
            >
              <span className='text-red-600 font-bold text-xl'>{redUpLevel}</span>
            </button>
            <button
              onClick={() => onUpgrade('blue')}
              className="w-10 h-10 bg-white hover:bg-slate-100 text-black rounded-md transition-colors flex items-center justify-center"
              title="蓝队升级"
            >
              <span className='text-blue-600 font-bold text-xl'>{blueUpLevel}</span>
            </button>
          </div>
          <button
