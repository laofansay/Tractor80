...
export default function Home() {
    ...
    if (currentPlayer && players[currentPlayer]?.isBot) {
      ...
      const aiTimeout = setTimeout(() => {
        ...
        // 出牌阶段的AI决策
          ...
          const cardsToPlay = selectCardsToPlay(aiCards, currentRoundData, trumpSuit);
  ...
  }, [currentPlayer, gamePhase, players, dealerPosition, trumpSuit]);
  ...
  const selectCardsToDiscard = (cards: string[], trumpSuit: string | null): string[] => {
    ...
    while (selectedCards.length < 6 && remainingCards.length > 0) {
      ...
      // 如果不是大小王，检查是否可以扣掉
      if (suit !== 'B' && suit !== 'J') {
        const remainingSuitCount = cards.filter(c => c.charAt(0) === suit && !selectedCards.includes(c)).length - 1;
      ...
      if (remainingCards.length > 0 && remainingCards[0] === card) {
        // 如果队列头部的牌没有变化，说明所有牌都不满足条件，直接选择分数最低的牌
  ...
  // 比较两张牌的大小
  const compareCards = (cardA: string, cardB: string, leadingSuit: string, trumpSuit: string | null): number => {
    const suitA = cardA.charAt(0);
    const suitB = cardB.charAt(0);
    const valueA = cardA.substring(1);
    const valueB = cardB.substring(1);

    // 大小王最大
    if (suitA === 'B' && suitB !== 'B') return 1;
    if (suitA !== 'B' && suitB === 'B') return -1;
    if (suitA === 'J' && suitB !== 'B' && suitB !== 'J') return 1;
    if (suitA !== 'B' && suitA !== 'J' && suitB === 'J') return -1;
    ...
    // 首出花色大于其他花色（除了主牌和大小王）
    if (suitA === leadingSuit && suitB !== leadingSuit && suitB !== trumpSuit && suitB !== 'B' && suitB !== 'J') return 1;
    if (suitA !== leadingSuit && suitA !== trumpSuit && suitA !== 'B' && suitA !== 'J' && suitB === leadingSuit) return -1;

    // 同花色比较点数
    if (suitA === suitB) {
      const numA = valueA === '1' ? 14 : parseInt(valueA); // A当作14点
      const numB = valueB === '1' ? 14 : parseInt(valueB);
      return numA - numB; // 返回正数表示A大于B
    }

    return 0; // 不同花色且都不是主牌或首出花色，无法比较
  };
