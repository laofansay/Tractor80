...
export default function Home() {
  ...
  const [availableSuits, setAavailableSuits] = useState < [] > (null);
  ...
  // 可选择的主牌花色
  const [availableSuits, setAvailableSuits] = useState < string[] > (['S', 'H', 'D', 'C']);
  ...
  const dealCards = () => {
    ...
    const dealNextCard = () => {
      if (cardIndex < 48) { // 48张牌给玩家（每人12张）
          ...
          if (newPlayers[position].isBot) {
            ...
            for (const suit of suits) {
              ...
              if (playerCards.includes(`${suit}2`)) {
                // 统计该花色的牌数量
                const sameSuitCards = playerCards.filter(card => card.charAt(0) === suit);
                // 检查是否有该花色的5、10或K
                const has5_10_K = playerCards.some(card =>
                  card.charAt(0) === suit && ['5', '10', 'K'].includes(card.substring(1)));
            ...
            //如果是玩家，设计一家可亮主的花色
            // 检查当前卡牌card的点数，花色
            const cardSuit = card.charAt(0);  // 花色
            const cardValue = card.substring(1); // 点数

            if (cardValue === '2') {
              setAavailableSuits(prevSuits => [...prevSuits, cardSuit]);
              setAavailableSuits([cardSuit]);
            }
            // 检查如果是王牌 
            if (cardValue === 'J' || cardValue === 'B') {
              //aavailableSuits追加一个值
              setAavailableSuits(prevSuits => [...prevSuits, 'NT']);

            }
            //暂停3秒，再执行后面的操作
            setTimeout(() => {
              console.log('可用花色: 3秒等等待亮主', availableSuits);
            }, 3000); // 3秒后重置可用花色
          }
          // 继续发下一张牌
        ...
        // 发牌完成后，如果还没有亮主，则设置为亮主阶段
        if (gamePhase === 'dealing') {
          setGamePhase('trumpSelection');
          setCurrentPlayer('north'); // 设置北方玩家为第一个可以亮主的玩家
        }
      }
    };
  ...
  const declareTrump = (position: Position) => {
    if (gamePhase !== 'trumpSelection' && gamePhase !== 'dealing') return;
    ...
    for (const suit of suits) {
      ...
      if (playerCards.includes(`${suit}2`)) {
        ...
        // 如果该花色牌数量至少有2张或有5、10、K中任意一张，则选择该花色作为主牌
        if (sameSuitCards.length >= 2 || has5_10_K) {
          selectedSuit = suit;
          break;
        }
    ...
    setTrumpSuit(selectedSuit); // 设置主牌花色
    setGamePhase('pickBottomCards');
