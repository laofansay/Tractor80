        ...
        {gamePhase === 'initial' && deck.length > 0 && (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <div className="relative w-24 h-32">
              {[...Array(Math.min(10, deck.length))].map((_, index) => (
                <div
                  key={index}
                  className="absolute w-24 h-32 bg-white rounded-md shadow-md border border-gray-300"
                  style={{
                ...
                >
                  <div className="w-full h-full bg-blue-600 rounded-md flex items-center justify-center">
                    <div className="w-16 h-24 border-2 border-white rounded opacity-50"></div>
                  </div>
                </div>
              ))}
