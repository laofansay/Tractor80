...
export default function Home() {
  ...
  const evaluateHandStrength = (cards: string[]): boolean => {
    ...
    const hasTwoInSuit: Record<string, boolean> = { 'S': false, 'H': false, 'D': false, 'C': false };
    const has5_10_K_InSuit: Record<string, boolean> = { 'S': false, 'H': false, 'D': false, 'C': false };
    
    // 统计每个花色的牌数量，以及是否有2和5、10、K
    cards.forEach(card => {
      const suit = card.charAt(0);
      const value = card.substring(1);
      ...
      if (suit === 'S' || suit === 'H' || suit === 'D' || suit === 'C') {
        ...
        // 检查是否有2
        if (value === '2') {
          hasTwoInSuit[suit] = true;
        }
        
        // 检查是否有5、10或K
        if (value === '5' || value === '10' || value === 'K') {
          has5_10_K_InSuit[suit] = true;
        }
      }
    });
    
    // 检查是否满足打2的亮主条件：有2且该花色至少有2张牌，或者有5/10/K之一
    for (const suit of ['S', 'H', 'D', 'C']) {
      if (hasTwoInSuit[suit] && (suitCounts[suit] >= 2 || has5_10_K_InSuit[suit])) {
        return true; // 满足条件，亮主
      }
    }
    
    // 如果不满足打2的条件，则使用原来的评估逻辑
