...
export default function Home() {
      ...
      if (cardIndex < 48) { // 48张牌给玩家（每人12张）
        ...
        if (card) {
          ...
          if (dealerPosition === null) {
            ...
            if (result.success && result.dealerPosition) {
              setDealerPosition(result.dealerPosition);
            }
            // 继续发下一张牌
  ...
  // 选择庄家并设置主牌花色，亮主
  const declareTrump = (position: Position, card?: string): { success: boolean; dealerPosition?: Position; trumpSuit?: string } => {
      ...
      for (const suit of suits) {
        ...
        if (playerCards.includes(suit + '2')) {
          ...
          if (sameSuitCards.length >= 1 && has5_10_K) {
            ...
            return { success: true, dealerPosition: position, trumpSuit: suit };
          }
        }
      }
    } else {
