...
export default function Home() {
  ...
  //监听玩家的手牌变化 自动亮主
  useEffect(() => {
    // 不再在这里调用declareTrump，因为已经在dealNextCard中调用了
    // 如果需要，可以保留此钩子用于其他逻辑
  ...
  // 选择庄家并设置主牌花色，亮主
  const declareTrump = (position: Position, newCard?: string) => {
    ...
    if (newPlayers[position].isBot) {
      ...
      for (const suit of suits) {
        ...
        if (playerCards.includes(suit + '2')) {
          ...
          if (sameSuitCards.length >= 7 && has5_10_K) {
