...
export default function Home() {
  ...
  const dealCards = () => {
    ...
    const dealNextCard = () => {
      ...
      const position = positions[cardIndex % 4];
      const card = obsCards.shift();
      if (!card) return;
      ...
      // 更新玩家手牌
      newPlayers[position].cards.push(card);
      newPlayers.obs.cards = obsCards;
  ...
  const declareTrump = (position: Position, card: string) => {
    if (dealerPosition !== null) return; // 已有庄家则直接返回
    ...
    if (newPlayers[position].isBot) {
      ...
      for (const suit of suits) {
        if (playerCards.includes(`${suit}2`)) {
          const sameSuitCards = playerCards.filter(c => c.startsWith(suit));
          const hasHighCard = playerCards.some(c => c.startsWith(suit) && ['5', '10', 'K'].includes(c.substring(1)));

          if (sameSuitCards.length >= 2 || hasHighCard) {
            setTrumpSuitAndDealer(suit, position, newPlayers);
            return;
          }
        }
      }
      ...
      setTrumpSuitAndDealer(cardSuit, position, newPlayers);
    }
  };

  const setTrumpSuitAndDealer = (suit: string, position: Position, newPlayers: any) => {
    newPlayers.obs.trumpSuit = suit;
    newPlayers[position].isDealer = true;
    newPlayers[position].isDeclareTrump = true;
