...
export default function Home() {
  ...
  // 选择庄家并设置主牌花色，亮主
  const declareTrump = (position: Position, card: Card) => {

    if (gamePhase !== 'trumpSelection' && gamePhase !== 'dealing' || dealerPosition != '') return;
    if (newPlayers[position].isBot) {
      ...
      if (cardValue === '2') {
        ...
        setAvailableSuits(prevSuits => [...prevSuits, cardSuit]);
      }
      // 检查如果是王牌 
      if (cardValue === 'J' || cardValue === 'B') {
        setAvailableSuits(prevSuits => [...prevSuits, 'NT']);
      }
      newPlayers.obs.trumpSuit = cardSuit;
      setDealerPosition(position)
      setPlayers({ ...newPlayers });
      //暂停3秒，再执行后面的操作
      setTimeout(() => {
