...
export default function Home() {
  ...
  const [players, setPlayers] = useState < Record < Position, Player>> ({
    ...
    obs: { cards: [], isDealer: false, isObs: true, camp: null, RecCards: [], currMcurrentRoundrgeback: [], lastRound: [] },
  });
  ...
  //当前回合
  // 移除独立的currentRound和lastRound状态，现在它们存储在obs对象中
  ...
  const playCard = (position: Position, cards: string[]) => {
    ...
    // 更新当前回合的出牌记录（存储在obs中）
    const newCurrentRound = { north: [], east: [], south: [], west: [], ...newPlayers.obs.currMcurrentRoundrgeback };
    newCurrentRound[position] = cards;
    newPlayers.obs = {
      ...newPlayers.obs,
      currMcurrentRoundrgeback: newCurrentRound
    };
    setPlayers(newPlayers);
    ...
    if (nextPlayer === getFirstPlayerOfRound()) {
      // 保存当前回合记录到obs的lastRound，并清空currentRound
      const newPlayers = { ...players };
      newPlayers.obs = {
        ...newPlayers.obs,
        lastRound: newCurrentRound,
  ...
  const dealCards = () => {
    ...
    const dealNextCard = () => {
      if (cardIndex < 48) { // 48张牌给玩家（每人12张）
        ...
        // 发完玩家的牌后，设置底牌
        const remainingCards = obsCards.slice(cardIndex, cardIndex + 6);
        setBottomCards(remainingCards);

        // 更新OBS玩家的cards（移除已经发出去的牌）
        newPlayers.obs.cards = [];
        setPlayers(newPlayers);
    ...
    if (currentPlayer && currentPlayer !== 'south') {
      ...
      const aiTimeout = setTimeout(() => {
        ...
        if (gamePhase === 'trumpSelection') {
          ...
          // 评估手牌强度决定是否亮主
          const shouldDeclare = evaluateHandStrength(currentPlayer, aiCards);
  ...
  const evaluateHandStrength = (position: Position, cards: string[]): boolean => {
    ...
    // 可以根据玩家位置调整策略
    // 例如：不同位置的玩家可能有不同的亮主倾向
    let positionFactor = 1.0;
    if (position === 'north' || position === 'south') {
      // 假设北家和南家是同一阵营，可能更倾向于亮主
      positionFactor = 1.2;
    }

    // 如果强牌数量超过阈值，则亮主
    // 随机因素：80%的概率按照强牌数量决定，20%的概率随机决定
    ...
    if (randomFactor) {
      ...
      // 考虑位置因素
      return (strongCards * positionFactor) >= 5 || Math.random() < 0.4; // 加入位置因素，并保留一定随机性
    }
  };
